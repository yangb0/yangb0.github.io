!function e(t, n, r) {
    ...
    1: [function (require, module, exports) {
        ...
        function fuzzysearch(needle, haystack) {
            var tlen = haystack.length, qlen = needle.length;
            if (qlen > tlen)return !1;
            if (qlen === tlen)return needle === haystack;
            outer:for (var i = 0, j = 0; i < qlen; i++) {
                for (var nch = needle.charCodeAt(i); j < tlen;)if (haystack.charCodeAt(j++) === nch)continue outer;
                return !1
            }
            return !0
        }
    ...
    4: [function (require, module, exports) {
        ...
        var FuzzySearchStrategy = require("./SearchStrategies/FuzzySearchStrategy"),
            LiteralSearchStrategy = require("./SearchStrategies/LiteralSearchStrategy"), data = [], opt = {};
    ...
    8: [function (require, module, exports) {
        !function (window, document, undefined) {
            ...
            var options = {
                    ...
                    fuzzy: !1,
                    exclude: []
                }, requiredOptions = ["searchInput", "resultsContainer", "json"], templater = require("./Templater"),
                repository = require("./Repository"), jsonLoader = require("./JSONLoader"),
...
!function () {
    if (/Android|webOS|iPhone|iPod|iPad|BlackBerry/i.test(navigator.userAgent))var t = 120; else var t = 320;
    var s = document.querySelectorAll(".docs-header")[0], i = window.innerWidth - 20, n = s.offsetHeight||0, a = 0, r = 0,
        h = 150, o = function (t) {
            ...
            parts: {
                sz: function (t, s) {
                    return {x: t.x * s.x, y: t.y * s.y, z: t.z * s.z}
                }, rot: {
                    x: function (t, s) {
                        return {
                            x: t.x,
                ...
                plane: function (t) {
                    return {x: t.x * d.ang.cplane + t.z * d.ang.splane, y: t.y, z: t.x * -d.ang.splane + t.z * d.ang.cplane}
                }, theta: function (t) {
                    return {x: t.x, y: t.y * d.ang.ctheta - t.z * d.ang.stheta, z: t.y * d.ang.stheta + t.z * d.ang.ctheta}
                }, set: function (t) {
                    return {x: t.x - d.obj.x, y: t.y - d.obj.y, z: t.z - d.obj.z}
                }
            }, persp: function (t) {
                return {
                ...
                return {x: t.x + s.x, y: -t.y + s.y, z: t.z + s.z, p: t.p}
    ...
    !function () {
        "use strict";
        var h = function (t) {
            this.transIn = {}, this.transOut = {}, this.transIn.vtx = t.vtx, this.transIn.sz = t.sz, this.transIn.rot = t.rot, this.transIn.pos = t.pos
        };
        h.prototype.vupd = function () {
            this.transOut = c.steps(this.transIn.vtx, this.transIn.sz, this.transIn.rot, this.transIn.pos, d.disp)
        };
        var o = function () {
            this.vel = .04, this.lim = 360, this.diff = 200, this.initPos = 100, this.toX = a, this.toY = r, this.go()
        };
        o.prototype.go = function () {
            this.canvas = document.getElementById("mycanvas"), this.canvas.width = window.innerWidth - 20, this.canvas.height = s.offsetHeight, this.$ = mycanvas.getContext("2d"), this.$.globalCompositeOperation = "source-over", this.varr = [], this.dist = [], this.calc = [];
            for (var a = 0, r = t; a < r; a++)this.add();
            this.rotObj = {x: 0, y: 0, z: 0}, this.objSz = {x: i / 5, y: n / 5, z: i / 5}
        }, o.prototype.add = function () {
            this.varr.push(new h({
                vtx: {x: e(), y: e(), z: e()},
                sz: {x: 0, y: 0, z: 0},
                rot: {x: 20, y: -20, z: 0},
                pos: {
        ...
        }, o.prototype.upd = function () {
            d.obj.x += .05 * (this.toX - d.obj.x), d.obj.y += .05 * (this.toY - d.obj.y)
        }, o.prototype.draw = function () {
            this.$.clearRect(0, 0, this.canvas.width, this.canvas.height), d.upd(), this.rotObj.x += .1, this.rotObj.y += .1, this.rotObj.z += .1;
            for (var t = 0; t < this.varr.length; t++) {
                for (var s in this.calc[t])this.calc[t].hasOwnProperty(s) && (this.calc[t][s] += this.vel, this.calc[t][s] > this.lim && (this.calc[t][s] = 0));
                if (this.varr[t].transIn.pos = {
                        x: this.diff * Math.cos(this.calc[t].x * Math.PI / 180),
                        y: this.diff * Math.sin(this.calc[t].y * Math.PI / 180),
                        z: this.diff * Math.sin(this.calc[t].z * Math.PI / 180)
                    }, this.varr[t].transIn.rot = this.rotObj, this.varr[t].transIn.sz = this.objSz, this.varr[t].vupd(), !(this.varr[t].transOut.p < 0)) {
                    var i = this.$.createRadialGradient(this.varr[t].transOut.x, this.varr[t].transOut.y, this.varr[t].transOut.p, this.varr[t].transOut.x, this.varr[t].transOut.y, 2 * this.varr[t].transOut.p);
                    this.$.globalCompositeOperation = "lighter", i.addColorStop(0, "hsla(255, 255%, 255%, 1)"), i.addColorStop(.5, "hsla(" + (t + 2) + ",85%, 40%,1)"), i.addColorStop(1, "hsla(" + t + ",85%, 40%,.5)"), this.$.fillStyle = i, this.$.beginPath(), this.$.arc(this.varr[t].transOut.x, this.varr[t].transOut.y, 2 * this.varr[t].transOut.p, 0, 2 * Math.PI, !1), this.$.fill(), this.$.closePath()
                }
            }
        }, o.prototype.anim = function () {
            window.requestAnimationFrame = function () {
                return window.requestAnimationFrame || function (t, s) {
                        window.setTimeout(t, 1e3 / 60)
                    }
            }();
            var t = function () {
                this.upd(), this.draw(), window.requestAnimationFrame(t)
            }.bind(this);
            window.requestAnimationFrame(t)
        }, o.prototype.run = function () {
            this.anim(), s.addEventListener("mousemove", function (t) {
                this.toX = (t.clientX - this.canvas.width / 2) * -.8, this.toY = .8 * (t.clientY - this.canvas.height / 2)
            }.bind(this)), s.addEventListener("touchmove", function (t) {
                t.preventDefault(), this.toX = (t.touches[0].clientX - this.canvas.width / 2) * -.8, this.toY = .8 * (t.touches[0].clientY - this.canvas.height / 2)
            }.bind(this))
        };
        var u = new o;
        u.run()
        ...
        mycanvas.width = i = window.innerWidth - 20, mycanvas.height = n = t.offsetHeight
    }, !1)
}();